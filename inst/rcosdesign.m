...
function b = rcosdesign (beta, span, sps, shape = "sqrt")
  ...
  if (strcmp (shape, "normal"))
    ...
    idx0 = (abs (t) < tol);
    ## Handle special case: |t| = 1/(2*beta)
  ...
  elseif (strcmp (shape, "sqrt"))
    ...
    if (beta > 0)
      ...
      others = !(idx0 | idx_inf);
      arg1 = pi * t(others) * (1 - beta);
      arg2 = pi * t(others) * (1 + beta);
      denom = pi * t(others) .* (1 - (4 * beta * t(others)).^2);
      b(others) = (sin (arg1) + 4 * beta * t(others) .* cos (arg2)) ./ denom;
...
%!demo
%! ## Compare normal and square-root raised cosine impulse responses
%! beta = 0.25;
%! span = 6;
%! sps = 4;
